...
namespace PaymentManager.Repositories
{
    ...
    {
        protected readonly PayableDbContext _context;

        public PayableRepository(PayableDbContext context)
        {
            _context = context;
        }
        ...
        {
            var toRemove = entities.Where(e => e.Paid == 0);
            _context.Payables.RemoveRange(toRemove);

            var toUpdate = entities.Where(e => e.Paid > 0);
